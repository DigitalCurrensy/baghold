# Waves

W0: two published bags (Mare Tranquillitatis pit, Marius Hills pit) vs declared hold stats. Scorer named, not run.

W1: hold scorer. `bag.py` ported. Pinch first.

W2: one bad bag demonstration. Famous is not a hold. Pretty-print is not a pass.

W3: hold letter. Paper on the bag. Fail still issues.

W4: counsel pass. Compiled from locked fields. Counsel unsigned.

Stop. Do not staff TUBEWALK.
